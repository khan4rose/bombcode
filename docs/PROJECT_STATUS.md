...
| Android viewport metrics 반복 / 자동 종료 | 확인 필요 | `WindowInsets changed`, `FlutterJNI: Sending viewport metrics`, `Lost connection to device` 로그 발생. `SafeArea(bottom:false)`/`MediaQuery.removePadding` 시도 후 롤백됨 |
...
- Android emulator에서 viewport metrics 반복과 device connection lost가 발생해 Android/window inset 원인 확인이 필요하다.
